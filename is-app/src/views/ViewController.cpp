...
ViewController::ViewController(Window *window)
    : GuiComponent(window), mCurrentView(nullptr),
      mCamera(Eigen::Affine3f::Identity()), mFadeOpacity(0), mLockInput(false) {
...
void ViewController::goToSystemView(SystemData *system) {
  ...
  systemList->setPosition(getSystemId(system) *
...
void ViewController::playViewTransition() {
  ...
  // no need to animate, we're not going anywhere (probably goToNextGamelist()
  // or goToPrevGamelist() when there's only 1 system)
  ...
  if (Settings::getInstance()->getString("TransitionStyle") == "fade") {
    ...
    auto fadeFunc = [this](float t) { mFadeOpacity = lerp<float>(0, 1, t); };
...
void ViewController::launch(FileData *game, Eigen::Vector3f center) {
  ...
  if (Settings::getInstance()->getString("TransitionStyle") == "fade") {
    ...
    setAnimation(new LaunchAnimation(mCamera, mFadeOpacity, center, 1500), 0,
      ...
      mCamera = origCamera;
      mLockInput = false;
      setAnimation(new LaunchAnimation(mCamera, mFadeOpacity, center, 600), 0,
                   nullptr, true);
      this->onFileChanged(game, FILE_METADATA_CHANGED);
    });
  }
...
std::shared_ptr<IGameListView>
ViewController::getGameListView(SystemData *system) {
  ...
  // view = std::shared_ptr<IGameListView>(new GridGameListView(mWindow,
  // system->getRootFolder()));
...
void ViewController::render(const Eigen::Affine3f &parentTrans) {
  ...
  Eigen::Vector3f viewEnd =
      trans.inverse() * Eigen::Vector3f((float)Renderer::getScreenWidth(),
  ...
  for (auto it = mGameListViews.begin(); it != mGameListViews.end(); it++) {
    ...
    Eigen::Vector3f guiEnd = it->second->getPosition() +
                             Eigen::Vector3f(it->second->getSize().x(),
  ...
  if (mFadeOpacity) {
    ...
    Renderer::drawRect(0, 0, Renderer::getScreenWidth(),
                       ...
                       0x00000000 | (unsigned char)(mFadeOpacity * 255));
  }
}
