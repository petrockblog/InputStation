// EmulationStation, a graphical front-end for ROM browsing. Created by Alec
// "Aloshi" Lofquist.
// http://www.aloshi.com
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--resolution") == 0) {
      ...
      *height = atoi(argv[i + 2]);
      ...
      bool vsync =
          (strcmp(argv[i + 1], "on") == 0 || strcmp(argv[i + 1], "1") == 0)
              ? true
              : false;
...
//     *errorString =
//         "IT LOOKS LIKE YOUR SYSTEMS CONFIGURATION FILE HAS NOT BEEN "
//         "SET UP OR IS INVALID. YOU'LL NEED TO DO THIS BY HAND, "
//         "UNFORTUNATELY.\n\n"
...
//     LOG(LogError) << "No systems found! Does at least one system have a game "
//                      "present? (check that extensions match!)\n(Also, make "
//                      "sure you've updated your es_systems.cfg for XML!)";
//     *errorString =
//         "WE CAN'T FIND ANY SYSTEMS!\n"
//         "CHECK THAT YOUR PATHS ARE CORRECT IN THE SYSTEMS CONFIGURATION FILE, "
//         "AND YOUR GAME DIRECTORY HAS AT LEAST ONE GAME WITH THE CORRECT "
//         "EXTENSION.\n\n"
//         "VISIT EMULATIONSTATION.ORG FOR MORE INFORMATION.";
//     return false;
//   }
...
int main(int argc, char* argv[]) {
  ...
  if (!parseArgs(argc, argv, &width, &height)) return 0;

  // if ~/.emulationstation doesn't exist and cannot be created, bail
  // if (!verifyHomeFolderExists()) return 1;
  ...
  //   if (errorMsg == NULL) {
  //     // LOG(LogError)
  //     //     << "Unknown error occured while parsing system config file.";
  //     if (!scrape_cmdline) Renderer::deinit();
  //     return 1;
  //   }

  //   // we can't handle es_systems.cfg file problems inside ES itself, so display
  //   // the error message then quit
  //   window.pushGui(new GuiMsgBox(&window, errorMsg, "QUIT", [] {
  ...
  // choose which GUI to open depending on if an input configuration already
  // exists
  if (errorMsg == NULL) {
    ...
    // } else {
      window.pushGui(new GuiDetectDevice(
          &window, true, [] { ViewController::get()->goToStart(); }));
    // }
  }
  ...
  while (window.peekGui() != ViewController::get()) delete window.peekGui();
  window.deinit();
