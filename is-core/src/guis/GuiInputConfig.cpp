...
static const int inputCount = 11;
static const char *inputName[inputCount] = {
    "Up",    "Down",   "Left",   "Right",    "A",    "B",
    "Start", "Select", "PageUp", "PageDown", "Katha"};
static const bool inputSkippable[inputCount] = {
    false, false, false, false, false, false, false, false, true, true, true};
static const char *inputDispName[inputCount] = {
    "UP",    "DOWN",   "LEFT",    "RIGHT",     "A",    "B",
    "START", "SELECT", "PAGE UP", "PAGE DOWN", "KATHA"};
static const char *inputIcon[inputCount] = {
    ":/help/dpad_up.svg",      ":/help/dpad_down.svg",
    ":/help/dpad_left.svg",    ":/help/dpad_right.svg",
    ":/help/button_a.svg",     ":/help/button_b.svg",
    ":/help/button_start.svg", ":/help/button_select.svg",
    ":/help/button_l.svg",     ":/help/button_r.svg",
    ":/help/button_r.svg"};

// MasterVolUp and MasterVolDown are also hooked up, but do not appear on this
// screen.
...
GuiInputConfig::GuiInputConfig(Window *window, InputConfig *target,
  ...
  mSubtitle1 = std::make_shared<TextComponent>(mWindow, strToUpper(ss.str()),
                                               Font::get(FONT_SIZE_MEDIUM),
                                               0x555555FF, ALIGN_CENTER);
  mGrid.setEntry(mSubtitle1, Vector2i(0, 2), false, true);

  mSubtitle2 = std::make_shared<TextComponent>(
      mWindow, "HOLD ANY BUTTON TO SKIP", Font::get(FONT_SIZE_SMALL),
      0x99999900, ALIGN_CENTER);
  ...
  for (int i = 0; i < inputCount; i++) {
    ...
    auto text = std::make_shared<TextComponent>(
        mWindow, inputDispName[i], Font::get(FONT_SIZE_MEDIUM), 0x777777FF);
    row.addElement(text, true);

    auto mapping = std::make_shared<TextComponent>(
        mWindow, "-NOT DEFINED-", Font::get(FONT_SIZE_MEDIUM, FONT_PATH_LIGHT),
    ...
    row.input_handler =
        [this, i, mapping](InputConfig *config, Input input) -> bool {
      ...
      if (!mConfiguringRow) {
        ...
        // we're not configuring and they didn't press A to start, so ignore
        // this
        return false;
      }
      ...
      if (input.value != 0) {
        // input down
        // if we're already holding something, ignore this, otherwise plan to
        // map this input
        if (mHoldingInput)
          return true;
        ...
        if (assign(mHeldInput, i))
          rowDone(); // if successful, move cursor/stop configuring - if not,
  ...
  setSize(Renderer::getScreenWidth() * 0.6f,
...
void GuiInputConfig::setAssignedTo(const std::shared_ptr<TextComponent> &text,
                                   Input input) {
...
bool GuiInputConfig::assign(Input input, int inputId) {
  ...
  // if this input is mapped to something other than "nothing" or the current
  // row, error
  // (if it's the same as what it was before, allow it)
  if (mTargetConfig->getMappedTo(input).size() > 0 &&
      !mTargetConfig->isMappedTo(inputName[inputId], input)) {
