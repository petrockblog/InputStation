...
// SO HEY POTENTIAL POOR SAP WHO IS TRYING TO MAKE SENSE OF ALL THIS (by which I
// mean my future self)
// There are like four distinct IDs used for joysticks (crazy, right?)
// 1. Device index - this is the "lowest level" identifier, and is just the Nth
// joystick plugged in to the system (like /dev/js#).
//    It can change even if the device is the same, and is only used to open
//    joysticks (required to receive SDL events).
// 2. SDL_JoystickID - this is an ID for each joystick that is supposed to
// remain consistent between plugging and unplugging.
//    ES doesn't care if it does, though.
// 3. "Device ID" - this is something I made up and is what InputConfig's
// getDeviceID() returns.
//    This is actually just an SDL_JoystickID (also called instance ID), but -1
//    means "keyboard" instead of "error."
// 4. Joystick GUID - this is some squashed version of joystick vendor, version,
// and a bunch of other device-specific things.
//    It should remain the same across runs of the program/system
//    restarts/device reordering and is what I use to identify which joystick to
//    load.
...
InputManager::~InputManager() { deinit(); }
...
void InputManager::init() {
  ...
  mKeyboardInputConfig =
      new InputConfig(DEVICE_KEYBOARD, "Keyboard", KEYBOARD_GUID_STRING);
...
void InputManager::deinit() {
  ...
  for (auto iter = mPrevAxisValues.begin(); iter != mPrevAxisValues.end();
       iter++) {
    delete[] iter -> second;
  }
...
bool InputManager::parseEvent(const SDL_Event &ev, Window *window) {
  ...
  case SDL_JOYAXISMOTION:
    ...
    if ((abs(ev.jaxis.value) > DEADZONE) !=
      ...
      window->input(
          getInputConfigByDevice(ev.jaxis.which),
  ...
  case SDL_JOYBUTTONUP:
    window->input(getInputConfigByDevice(ev.jbutton.which),
                  Input(ev.jbutton.which, TYPE_BUTTON, ev.jbutton.button,
                        ev.jbutton.state == SDL_PRESSED, false));
    return true;

  case SDL_JOYHATMOTION:
    window->input(
        getInputConfigByDevice(ev.jhat.which),
  ...
  case SDL_JOYDEVICEADDED:
    addJoystickByDeviceIndex(
        ev.jdevice.which); // ev.jdevice.which is a device index
    return true;

  case SDL_JOYDEVICEREMOVED:
    removeJoystickByJoystickID(
        ev.jdevice.which); // ev.jdevice.which is an SDL_JoystickID (instance ID)
    return false;
  }
...
bool InputManager::loadInputConfig(InputConfig *config) {
  ...
  if (!configNode)
    configNode = root.find_child_by_attribute("inputConfig", "deviceName",
...
// used in an "emergency" where no keyboard config could be loaded from the
// inputmanager config file
// allows the user to select to reconfigure in menus if this happens without
...
void InputManager::loadDefaultKBConfig() {
  ...
  cfg->mapInput("pageup",
                Input(DEVICE_KEYBOARD, TYPE_KEY, SDLK_RIGHTBRACKET, 1, true));
  cfg->mapInput("pagedown",
                Input(DEVICE_KEYBOARD, TYPE_KEY, SDLK_LEFTBRACKET, 1, true));
}
...
std::string InputManager::getConfigPath() {
  ...
  path += "/inputconfiguration.cfg";
  return path;
}

bool InputManager::initialized() const { return mKeyboardInputConfig != NULL; }
...
std::string InputManager::getDeviceGUIDString(int deviceId) {
  ...
  if (it == mJoysticks.end()) {
    LOG(LogError) << "getDeviceGUIDString - deviceId " << deviceId
                  << " not found!";
