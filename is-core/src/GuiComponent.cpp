...
GuiComponent::GuiComponent(Window *window)
    : mWindow(window), mParent(NULL), mOpacity(255),
      mPosition(Eigen::Vector3f::Zero()), mSize(Eigen::Vector2f::Zero()),
...
Eigen::Vector3f GuiComponent::getPosition() const { return mPosition; }
...
Eigen::Vector2f GuiComponent::getSize() const { return mSize; }
...
void GuiComponent::clearChildren() { mChildren.clear(); }

unsigned int GuiComponent::getChildCount() const { return mChildren.size(); }

GuiComponent *GuiComponent::getChild(unsigned int i) const {
  return mChildren.at(i);
}

void GuiComponent::setParent(GuiComponent *parent) { mParent = parent; }

GuiComponent *GuiComponent::getParent() const { return mParent; }

unsigned char GuiComponent::getOpacity() const { return mOpacity; }
...
std::string GuiComponent::getValue() const { return ""; }
...
void GuiComponent::setAnimation(Animation *anim, int delay,
                                ...
                                bool reverse, unsigned char slot) {
  assert(slot < MAX_ANIMATIONS);

  AnimationController *oldAnim = mAnimationMap[slot];
  mAnimationMap[slot] =
      new AnimationController(anim, delay, finishedCallback, reverse);
...
bool GuiComponent::finishAnimation(unsigned char slot) {
  ...
  if (mAnimationMap[slot]) {
    ...
    const bool done = mAnimationMap[slot]->update(
        mAnimationMap[slot]->getAnimation()->getDuration() -
        mAnimationMap[slot]->getTime());
    assert(done);
...
void GuiComponent::applyTheme(const std::shared_ptr<ThemeData> &theme,
  ...
  Eigen::Vector2f scale =
      getParent() ? getParent()->getSize()
                  : Eigen::Vector2f((float)Renderer::getScreenWidth(),
  ...
  if (properties & POSITION && elem->has("pos")) {
    Eigen::Vector2f denormalized =
        elem->get<Eigen::Vector2f>("pos").cwiseProduct(scale);
...
HelpStyle GuiComponent::getHelpStyle() { return HelpStyle(); }
