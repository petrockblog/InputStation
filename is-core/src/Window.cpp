...
Window::Window()
    : mNormalizeNextUpdate(false), mFrameTimeElapsed(0), mFrameCountElapsed(0),
      mAverageDeltaTime(10), mAllowSleep(true), mSleeping(false),
...
void Window::removeGui(GuiComponent *gui) {
  for (auto i = mGuiStack.begin(); i != mGuiStack.end(); i++) {
    if (*i == gui) {
      ...
      if (i == mGuiStack.end() &&
          mGuiStack
              .size()) // we just popped the stack and the stack is not empty
...
bool Window::init(unsigned int width, unsigned int height) {
  ...
  // keep a reference to the default fonts, so they don't keep getting
  // destroyed/recreated
...
void Window::input(InputConfig *config, Input input) {
  ...
  if (config->getDeviceId() == DEVICE_KEYBOARD && input.value &&
      input.id == SDLK_g && SDL_GetModState() & KMOD_LCTRL &&
      Settings::getInstance()->getBool("Debug")) {
    // toggle debug grid with Ctrl-G
    Settings::getInstance()->setBool(
        "DebugGrid", !Settings::getInstance()->getBool("DebugGrid"));
  } else if (config->getDeviceId() == DEVICE_KEYBOARD && input.value &&
             input.id == SDLK_t && SDL_GetModState() & KMOD_LCTRL &&
             Settings::getInstance()->getBool("Debug")) {
    // toggle TextComponent debug view with Ctrl-T
    Settings::getInstance()->setBool(
        "DebugText", !Settings::getInstance()->getBool("DebugText"));
  } else {
    if (peekGui())
  ...
  if (mFrameTimeElapsed > 500) {
    ...
    if (Settings::getInstance()->getBool("DrawFramerate")) {
      ...
      ss << std::fixed << std::setprecision(1)
         << (1000.0f * (float)mFrameCountElapsed / (float)mFrameTimeElapsed)
         << "fps, ";
      ss << std::fixed << std::setprecision(2)
         << ((float)mFrameTimeElapsed / (float)mFrameCountElapsed) << "ms";

      // vram
      float textureVramUsageMb =
          TextureResource::getTotalMemUsage() / 1000.0f / 1000.0f;
      ;
      float fontVramUsageMb = Font::getTotalMemUsage() / 1000.0f / 1000.0f;
      ;
      float totalVramUsageMb = textureVramUsageMb + fontVramUsageMb;
      ss << "\nVRAM: " << totalVramUsageMb << "mb (texs: " << textureVramUsageMb
         << "mb, fonts: " << fontVramUsageMb << "mb)";

      mFrameDataText =
          std::unique_ptr<TextCache>(mDefaultFonts.at(1)->buildTextCache(
              ss.str(), 50.f, 50.f, 0xFF00FFFF));
    }
...
void Window::render() {
  ...
  unsigned int screensaverTime =
      (unsigned int)Settings::getInstance()->getInt("ScreenSaverTime");
...
void Window::normalizeNextUpdate() { mNormalizeNextUpdate = true; }

bool Window::getAllowSleep() { return mAllowSleep; }

void Window::setAllowSleep(bool sleep) { mAllowSleep = sleep; }

void Window::renderLoadingScreen() {
  Eigen::Affine3f trans = Eigen::Affine3f::Identity();
  Renderer::setMatrix(trans);
  Renderer::drawRect(0, 0, Renderer::getScreenWidth(),
                     Renderer::getScreenHeight(), 0xFFFFFFFF);
  ...
  splash.setPosition((Renderer::getScreenWidth() - splash.getSize().x()) / 2,
                     (Renderer::getScreenHeight() - splash.getSize().y()) / 2 *
                         0.6f);
  splash.render(trans);

  auto &font = mDefaultFonts.at(1);
  TextCache *cache = font->buildTextCache("LOADING...", 0, 0, 0x656565FF);
  trans = trans.translate(Eigen::Vector3f(
      round((Renderer::getScreenWidth() - cache->metrics.size.x()) / 2.0f),
...
void Window::setHelpPrompts(const std::vector<HelpPrompt> &prompts,
  ...
  for (auto it = prompts.begin(); it != prompts.end(); it++) {
    ...
    if (inputSeenMap.insert(std::make_pair<std::string, bool>(it->first, true))
            .second) {
      ...
      if (mappedTo != mappedToSeenMap.end()) {
        ...
        // no, it hasn't!
        mappedToSeenMap.insert(
            std::pair<std::string, int>(it->second, addPrompts.size()));
        addPrompts.push_back(*it);
      }
    }
  }

  // sort prompts so it goes [dpad_all] [dpad_u/d] [dpad_l/r] [a/b/x/y/l/r]
  // [start/select]
  std::sort(addPrompts.begin(), addPrompts.end(),
            [](const HelpPrompt &a, const HelpPrompt &b) -> bool {

    static const char *map[] = {
        "up/down/left/right", "up/down", "left/right", "a",     "b",      "x",
        "y",                  "l",       "r",          "start", "select", NULL};
...
void Window::onSleep() {
  ...
  unsigned char opacity =
      Settings::getInstance()->getString("ScreenSaverBehavior") == "dim" ? 0xA0
                                                                         : 0xFF;
  Renderer::drawRect(0, 0, Renderer::getScreenWidth(),
